...
void RGBConverter::rgbDoubleToESPInt_single(double x, unsigned int* _x) {
  *_x = floor(x == 1.0 ? 1023 : x * 1024.0);
}
...
void RGBConverter::rgbDoubleToESPInt(double r, double g, double b, unsigned int* _r, unsigned int* _g, unsigned int* _b) {
  rgbDoubleToESPInt_single(r, _r);
  rgbDoubleToESPInt_single(g, _g);
  rgbDoubleToESPInt_single(b, _b);
}
